...
type MoviesAPI =
    ...
    "register" :> ReqBody '[JSON] RegisterMessage :> Post '[JSON] Token
...
impl = 
    ...
    accessError . liftIO . uncurry register . processRegister
    ...
    where
        ...
        processRegister registerMessage =
            (user registerMessage, passOrToken registerMessage)
    
        processAddMovie internalFunction movieToken =
            internalFunction (token movieToken)
            (nameOfMovie . movies $ movieToken) (rating.movies $ movieToken)

impl2 :: Server MoviesAPI
impl2 = register2 :<|> addMovie2 :<|> findMovies2
...
{- moviesAPILift = accessError . liftIO
    
    where accessError = (=<<) $ \case
            Nothing -> throwError err403
            Just content -> pure content -}
...
register2 :: RegisterMessage -> Handler Token
register2 registerMessage = process registerMessage
...
addMovie2 :: MovieToken -> Handler ()
addMovie2 movieToken = process movieToken
...
addMovie entryToken movieName movieRating =
    ...
    case hashCheck entryToken existingUserList of
        False -> invalidToken
        True | alreadyInList existingMoviesList -> pure <$>
...
findMovies2 :: Maybe Token -> Maybe String -> Handler [Movie]
findMovies2 tokArg queryArg = undefined

findMovies :: Maybe Token -> Maybe String -> IO (Maybe [Movie])
findMovies Nothing queryArg = noToken

    where

        noToken = pure Nothing

findMovies (Just tokArg) queryArg = 
        ...
        case elem tokArg registry of
            ...
            True -> 
                ...
                let filteredAndConvertedMovieList = movies <$>
                        filter ((==tokArg).token) movieList in
            
                pure.pure $ findMoviesInner
                    queryArg filteredAndConvertedMovieList

            where

                noTokenFound = pure Nothing
        
                findMoviesInner Nothing movieList = []

                findMoviesInner (Just queryArg) movieList =      
                    filter ( parse queryArg . nameOfMovie ) movieList
